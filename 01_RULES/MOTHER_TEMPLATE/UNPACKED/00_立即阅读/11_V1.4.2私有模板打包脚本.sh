...
zip_path="$out_dir/template-v142-private.zip"
sha_path="$out_dir/template-v142-private.sha256"
...
for filename in payment_required:
    if not any(name.endswith("/05_固定第三方能力与真实配置/02_支付/" + filename) for name in names):
        raise SystemExit("zip is missing V1.4.2 payment contract: " + filename)
print("ZIP_VALIDATE_PASS entries=%d duplicates=0 dangerous_paths=0 baseline_references=3 payment_contracts=6" % len(names))
PY
